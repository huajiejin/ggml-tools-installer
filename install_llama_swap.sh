#!/bin/bash

# Exit on any error
set -e

# --- Dependency Management ---
# Function to install Homebrew if not found
install_brew() {
    if ! command -v brew &> /dev/null; then
        echo "Homebrew not found. Installing Homebrew..."
        /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"
        # Add brew to path for this script's session
        eval "$(/opt/homebrew/bin/brew shellenv)"
    else
        echo "Homebrew is already installed."
    fi
}

# Function to check and install packages using Homebrew
check_and_install_packages() {
    install_brew
    for pkg in "$@"; do
        if ! command -v "$pkg" &> /dev/null; then
            echo "$pkg not found. Installing with Homebrew..."
            brew install "$pkg"
        else
            echo "$pkg is already installed."
        fi
    done
}
# --- End of Dependency Management ---

# Check for required tools
check_and_install_packages "git" "node" "go"

# Set project variables
REPO_URL="git@github.com:mostlygeek/llama-swap.git"
INSTALL_DIR="$HOME/.llama-swap"
BIN_DIR="$INSTALL_DIR/build"
BRANCH_NAME="main"
ALIAS_CMD="alias llama-swap='llama-swap-darwin-arm64'"

# Clone or update the repository
if [ -d "$INSTALL_DIR" ]; then
    echo "Updating llama-swap..."
    cd "$INSTALL_DIR"
    git checkout $BRANCH_NAME && git pull || { echo "Failed to update repository. Please check for local changes or network issues."; exit 1; }
else
    echo "Cloning llama-swap..."
    git clone "$REPO_URL" "$INSTALL_DIR" || { echo "Failed to clone repository. Please ensure your SSH key is configured on GitHub."; exit 1; }
    cd "$INSTALL_DIR"
fi

# Build the project
echo "Building llama-swap..."
(cd ui && npm install)
make clean all

# Add to shell profile if not already present
SHELL_PROFILES=("$HOME/.zshrc" "$HOME/.bashrc" "$HOME/.bash_profile" "$HOME/.profile")
EXPORT_PATH="export PATH=\"\$PATH:$BIN_DIR\""
CONFIG_ADDED=false

for PROFILE in "${SHELL_PROFILES[@]}"; do
    if [ -f "$PROFILE" ]; then
        PROFILE_UPDATED=false
        if ! grep -qF -- "$BIN_DIR" "$PROFILE"; then
            echo "Adding llama-swap to your PATH in $PROFILE"
            echo -e "\n# Add llama-swap to PATH\n$EXPORT_PATH" >> "$PROFILE"
            PROFILE_UPDATED=true
        fi
        if ! grep -qF -- "alias llama-swap" "$PROFILE"; then
            echo "Adding llama-swap alias in $PROFILE"
            echo -e "\n# Alias for llama-swap\n$ALIAS_CMD" >> "$PROFILE"
            PROFILE_UPDATED=true
        fi
        if [ "$PROFILE_UPDATED" = true ]; then
            CONFIG_ADDED=true
            break # Stop after updating the first found profile
        fi
    fi
done

if [ "$CONFIG_ADDED" = true ]; then
    echo "Please restart your terminal or run 'source <your_shell_profile>' to apply changes."
else
    echo "llama-swap configuration is already in your shell profile."
fi

echo "llama-swap installation complete. Binaries are in: $BIN_DIR"
